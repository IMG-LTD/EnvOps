...
public interface DeployTaskMapper {
  ...
  @Select({
      ...
      "  <if test='status != null'>",
      "    <choose>",
      "      <when test='status == \"RUNNING\"'>AND dt.status IN ('RUNNING', 'CANCEL_REQUESTED')</when>",
      "      <otherwise>AND dt.status = #{status}</otherwise>",
      "    </choose>",
      "  </if>",
      "  <if test='taskType != null'>AND dt.task_type = #{taskType}</if>",
      "  <if test='appId != null'>AND dt.app_id = #{appId}</if>",
      "  <if test='keyword != null'>",
      "    AND (dt.task_no LIKE CONCAT('%', #{keyword}, '%')",
      "      OR dt.task_name LIKE CONCAT('%', #{keyword}, '%')",
      "      OR ad.app_name LIKE CONCAT('%', #{keyword}, '%'))",
      "  </if>",
      ...
      "        ))",
      "        FROM deploy_task_param dtp",
      "        WHERE dtp.task_id = dt.id",
      "          AND dtp.secret_flag = 0",
      "          AND dtp.param_key IN ('environment', 'env', 'profile', 'namespace')",
      "      ) IN ('prod', 'production'))",
      "      OR (#{environment} = 'staging' AND (",
      "        SELECT LOWER(COALESCE(",
      "          NULLIF(TRIM(MAX(CASE WHEN dtp.param_key = 'environment' THEN dtp.param_value END)), ''),",
      "          NULLIF(TRIM(MAX(CASE WHEN dtp.param_key = 'env' THEN dtp.param_value END)), ''),",
      "          NULLIF(TRIM(MAX(CASE WHEN dtp.param_key = 'profile' THEN dtp.param_value END)), ''),",
      ...
      "          AND dtp.secret_flag = 0",
      "          AND dtp.param_key IN ('environment', 'env', 'profile', 'namespace')",
      "      ) IN ('stage', 'staging', 'pre', 'preprod', 'uat'))",
      "      OR (#{environment} = 'sandbox' AND (",
      "        SELECT LOWER(COALESCE(",
      "          NULLIF(TRIM(MAX(CASE WHEN dtp.param_key = 'environment' THEN dtp.param_value END)), ''),",
      "          NULLIF(TRIM(MAX(CASE WHEN dtp.param_key = 'env' THEN dtp.param_value END)), ''),",
      "          NULLIF(TRIM(MAX(CASE WHEN dtp.param_key = 'profile' THEN dtp.param_value END)), ''),",
      ...
      "          AND dtp.secret_flag = 0",
      "          AND dtp.param_key IN ('environment', 'env', 'profile', 'namespace')",
      "      ) IN ('sandbox', 'test', 'testing', 'dev', 'development'))",
      "    )",
      "  </if>",
      "  <if test='createdFrom != null'>AND dt.created_at <![CDATA[>=]]> #{createdFrom}</if>",
      "  <if test='createdTo != null'>AND dt.created_at <![CDATA[<=]]> #{createdTo}</if>",
      "</where>",
      "</script>"
  })
  ...
  @Select({
      ...
      "ORDER BY",
      "<choose>",
      "  <when test='sortBy == \"updatedAt\"'>dt.updated_at</when>",
      "  <when test='sortBy == \"taskNo\"'>dt.task_no</when>",
      "  <when test='sortBy == \"status\"'>dt.status</when>",
      "  <otherwise>dt.created_at</otherwise>",
      "</choose>",
      "<choose>",
  ...
  List<DeployTaskRow> findByQuery(@Param("keyword") String keyword,
                                  @Param("status") String status,
                                  @Param("taskType") String taskType,
                                  @Param("appId") Long appId,
                                  @Param("environment") String environment,
  ...
  @Select({
      ...
      "    </choose>",
      "  </if>",
      "  <if test='sourceType != null'>AND 'DEPLOY' = #{sourceType}</if>",
      "  <if test='taskType != null'>AND dt.task_type = #{taskType}</if>",
      "  <if test='priority != null'>",
      "    AND (CASE",
      "      WHEN dt.status = 'FAILED' OR dt.fail_count > 0 THEN 'P1'",
      "      WHEN dt.status IN ('RUNNING', 'CANCEL_REQUESTED') OR dt.target_count >= 10 THEN 'P2'",
      "      ELSE 'P3'",
      "    END) = #{priority}",
      "  </if>",
      "</where>",
  ...
  long countTaskCenterByQuery(@Param("keyword") String keyword,
                              @Param("status") String status,
  ...
  List<DeployTaskRow> findTaskCenterByQuery(@Param("keyword") String keyword,
                                            @Param("status") String status,
                                            @Param("sourceType") String sourceType,
                                            @Param("taskType") String taskType,
                                            @Param("priority") String priority,
