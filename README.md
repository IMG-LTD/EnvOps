...
- Task Center 当前提供最小真实统一任务中心，统一纳入 Deploy、数据库连通性检测、Traffic 动作三类任务，并统一使用 `pending`、`running`、`success`、`failed` 状态口径。
...
- [Release Notes](release/0.0.6-release-notes.md)
...
2. **统一任务中心**
   ...
   - Deploy 历史通过统一投影补录；数据库检测与 Traffic 动作从当前版本起记录新增任务；数据库批量检测按一批一条记录。
   - 前端交互采用“统一列表 + 轻量详情抽屉 + 查看原始详情深链”。
   - 当前范围仅覆盖统一列表、轻量详情抽屉、原模块深链，不新增任务内重试/取消、多任务编排、数据库与 Traffic 既有历史补录或新的执行器抽象层。
