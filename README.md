...
- 资产中心当前只覆盖主机、凭据、分组、标签四类资产入口，不承诺数据库资源、数据库实例或数据库专用生命周期。
...
1. **资产 / 检测 / 应用 / Deploy**
   - 保持当前版本的主要联调与演示能力。
   - 资产中心当前只提供 Host、Credential、Group、Tag 四类能力，其中 Host 与 Credential 支持创建和列表，Group 与 Tag 仅提供只读列表。
   - 当前没有数据库资源、数据库实例、数据库专用凭据或数据库生命周期管理入口，不应把“资产管理”理解成已支持数据库纳管。
