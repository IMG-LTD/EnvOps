...
const canManageConfigTemplates = computed(() => hasAuth('app:config-template:manage'));
...
function handleAdd() {
  if (!canManageConfigTemplates.value) {
    return;
  }
  ...
  <NSpace vertical :size="16">
    <NCard :bordered="false" class="card-wrapper" :title="$t('route.app_config-template')">
      <NSpace justify="space-between" align="center" wrap>
        ...
        <NSpace>
          <NButton type="primary" :disabled="!canManageConfigTemplates" @click="handleAdd">
            {{ $t('common.add') }}
          </NButton>
        ...
        <div v-for="item in templates" :key="item.id" class="rounded-12px bg-#f8f8f8 p-16px">
          <div class="flex items-start justify-between gap-12px">
            ...
            <div class="flex gap-8px">
              <NButton text type="primary" :disabled="!canManageConfigTemplates" @click="handleEdit(item)">
                {{ $t('common.edit') }}
              </NButton>
              <NButton text type="error" :disabled="!canManageConfigTemplates" @click="handleDelete(item)">
                {{ $t('common.delete') }}
              </NButton>
            </div>
          </div>
    ...
    <NCard
      ...
      :title="
        editingTemplateId === null
          ? $t('page.app.configTemplate.formTitleCreate')
          : $t('page.app.configTemplate.formTitleEdit')
      "
    >
      ...
      <NSpace justify="end">
        ...
        <NButton type="primary" :loading="submitting" :disabled="!canManageConfigTemplates" @click="handleSubmit">
