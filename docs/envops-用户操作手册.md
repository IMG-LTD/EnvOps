...
本文档面向当前基线的本地体验者、联调人员与演示使用者。当前阶段重点在于同步本地启动方式、统一任务中心的可体验范围，以及 Traffic 最小真实能力的使用口径。
...
页面支持按关键词、任务类型、状态、开始时间范围进行筛选。点击任务后会先打开轻量详情抽屉，查看基础信息、摘要、失败原因或批量汇总信息；如果需要更深内容，可继续点击“查看原始详情”跳回原模块。

当前体验边界如下：

- Deploy 历史任务已通过统一投影补录到列表中
- 数据库检测与 Traffic 动作从当前版本起记录新增任务
- 数据库批量检测按一批一条记录展示，不会拆成多条任务
- 任务中心当前只提供统一列表、轻量详情抽屉与原模块跳转，不提供任务内重试、取消、多任务编排或数据库与 Traffic 既有历史补录
...
6. 点击任一任务打开轻量详情抽屉，并通过“查看原始详情”跳回对应原模块
7. 打开 Traffic 页面，确认支持范围 warning、动作按钮可用性，以及最近一次动作反馈
